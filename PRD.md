# 다인가구: 1인 가구 외로움을 줄이는 공개 일상 로그 PRD
...
다인가구는 혼자 사는 사람들이 익명 닉네임으로 매시간의 2~4초 짧은 일상 영상을 남기고, 같은 동네/생활권에 있는 사람을 발견해 밥, 산책, 퇴근 후 수다 같은 가벼운 번개방으로 연결되는 모바일 웹 서비스다.

핵심 문장:

> 혼자 있는 순간을, 같이 있는 장면으로 바꾸는 공개 일상 로그.

해커톤 주제는 "1인가구 - 외로움"이며, 요구 방향은 "멋진 기술보다 실제로 매일 쓰일 한 줄의 도구"다. 다인가구 MVP는 매일 쓸 수 있는 이유를 세 가지로 압축한다.

1. 지금 나만 혼자인 게 아니라는 공개 신호를 본다.
2. 밥 먹을 사람, 산책할 사람을 N시간 안에 가볍게 찾는다.
3. 실제로 만나지 못해도 각자의 방에서 올린 짧은 영상 로그를 AI로 "함께 있던 한 장면"으로 남긴다.

이 서비스는 원본 다인가구 앱의 "친구들과 시시콜콜한 일상을 나누는 시간대별 기록" 경험에서 출발하되, 1인 가구의 외로움 문제에 맞춰 익명 동네 피드, 번개방 카드, 친구 채팅, 공유 사진첩, AI 단체 사진 생성으로 확장한다. 동네 기반 기능은 해커톤 MVP에서 실제 위치 매칭이 아니라 UI와 seed 데이터로만 제공한다.
...
하루에 한 번 이상 로그를 올리거나 번개에 참여한 사용자 비율.
...
- 공개 시간대별 로그 피드.
- 동네 기반 다인가구 UI. 실제 GPS/거리 계산은 구현하지 않고 동네 라벨과 seed 데이터로만 보여준다.
- 필터: 전체, 친구, 동성, 주변, 밥 번개.
- 로그 업로드: 2~4초 짧은 영상 로그. AI 생성 입력은 영상의 썸네일/대표 프레임을 사용한다.
...
- 친구 채팅.
- 친구/그룹 사진첩.
- 친구 또는 그룹 단위 로그 영상을 하나의 장소 이미지로 합성한 AI 단체 사진 생성.
...
7. 영상통화는 제공하지 않는다. 퇴근 후 수다방은 텍스트 채팅과 로그 공유만 제공한다.
...
1. 사진첩에서 친구 또는 그룹의 로그 영상 여러 개를 선택한다.
2. 각 영상의 대표 프레임/썸네일을 AI 입력 이미지로 사용한다.
3. 기준 장소가 될 다인가구를 하나 고른다.
...
- 방 카드에는 방 제목, 익명 호스트 닉네임, 동네 라벨, 참여자 수, 만료 시간, 대표 로그 썸네일을 표시한다.
...
- 로그 피드는 영상/사진이 주인공이어야 하며 장식 그래픽을 남발하지 않는다.
...
- 친구 피드에는 친구의 public/friends 로그 중 승인된 것만 노출.
- 시간대별 정렬은 최신 시간대 우선.
...
- 전체: 승인된 공개 로그.
- 친구: 친구의 승인된 로그.
- 동성: 현재 사용자와 gender가 같은 공개 로그.
- 주변: 현재 사용자와 cityLabel이 같은 공개 로그.
- 밥 번개: 활성 상태의 `meal_room` 번개방 및 관련 다인가구.
...
사진첩은 친구와 공유된 다인가구 및 AI 생성 이미지를 모아 보는 공간이다.

규칙:

- 친구별 사진첩과 그룹 사진첩 UI를 제공한다.
- 사용자는 여러 로그 영상을 선택할 수 있다.
- 영상 다인가구는 MVP에서 썸네일 또는 대표 프레임만 AI 생성 입력으로 사용한다.
...
- baseLogId: 기준 장소가 될 로그 영상의 대표 프레임.
- participantLogIds: 합성할 사람들의 로그 영상 대표 프레임 목록.
- style: realistic.
...
MVP에서는 친구 또는 그룹 사진첩에 포함된 로그만 AI 합성 대상으로 선택할 수 있다. 공개 피드의 모르는 사용자를 임의로 합성하는 기능은 제공하지 않는다.
...
- response: created 다인가구 with moderationStatus.
...
- 2~4초 로그 영상 업로드 플로우가 있다.
- 혼밥방, 퇴근 후 수다방, 동네 산책방 카드 생성과 참여가 가능하다.
- 친구 추가 후 친구 피드로 전환 가능하다.
- 친구 채팅이 가능하다.
- 사진첩에서 친구/그룹 로그 영상 대표 프레임을 활용한 AI 단체 사진 생성 플로우가 가능하다.
...
2. "다인가구는 지금 이 시간 같은 동네에서 혼자 있는 사람들의 2~4초 일상 로그를 보여줍니다."
3. 익명 닉네임과 동네 라벨이 붙은 혼밥 다인가구를 보여준다.
4. 동성/주변/친구 필터를 전환한다.
5. 혼밥방, 퇴근 후 수다방, 동네 산책방 카드 중 하나를 만든다.
6. 다른 사용자가 참여하고 채팅방이 열린다. 영상통화는 제공하지 않는다.
7. 서로 다른 방에서 밥 먹는 로그 영상 대표 프레임들을 선택한다.
...
- AI 필수성: 각자 다른 공간의 로그 프레임을 하나의 기억 이미지로 합성.
...
- 다인가구 streak 및 시간대별 회고.

## 18. References

- 다인가구 App Store: https://apps.apple.com/kr/app/setlog/id6587576438
